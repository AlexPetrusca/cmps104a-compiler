...
void lexer::dumplexeme(FILE* out, astree* lexeme) {
    fprintf (out, "%3d%4d.%03d%5d  %-15s  (%s)",
             (int) lexeme->lloc.filenr,
             (int) lexeme->lloc.linenr,
             (int) lexeme->lloc.offset,
             lexeme->symbol,
             parser::get_tname(lexeme->symbol),
             lexeme->lexinfo->c_str());
    fprintf (out, "\n");
}
